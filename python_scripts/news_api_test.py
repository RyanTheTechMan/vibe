...
def main():
    ...
    stock_id = 'AAPL'  # Change this to any stock ticker dynamically
    ...
    if articles:
        ...
        # Convert to a DataFrame for easier handling and exporting
        if data_for_db:
            df = pd.DataFrame(data_for_db)
            print("\n--- Data Prepared for Database ---")
            print(df)

            # Save to CSV or database (example saving to CSV)
